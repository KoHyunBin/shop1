...
public class UserController {
	...
	*/
	...
	//로그인 상태, 본인정보만 조회 검증 => AOP 클래스(UserLoginAspect)
	...
	@PostMapping("update")
	public ModelAndView idCheckupdate(@Valid User user, BindingResult bresult, String userid, HttpSession session) throws Throwable {
		ModelAndView mav = new ModelAndView();
		//입력값 검증
		if(bresult.hasErrors()) {
			mav.getModel().putAll(bresult.getModel());
			bresult.reject("error.update.input");
			return mav;
		}
		//비밀번호 검증
	...
	public String idCheckdelete(String password, String userid, HttpSession session) {
		...
		if(userid.equals("admin")) {
			throw new LoginException("관리자는 탈퇴 못함","mypage?userid="+userid);
		}
		// 비밀번호 검증 : 로그인된 정보
		User loginUser = (User)session.getAttribute("loginUser");
		//password : 입력된 비밀번호
	...
	/*
	 * 1.로그인 검증 - AOP 클래스 UserLoginAspect.loginCheck() : UserController.loginCheck*(..) 인 메서드
